...
class DiagnosisCubit extends Cubit<DiagnosisState> {
  ...
  DiagnosisCubit({required DiagnosisRepo repo})
      : _repo = repo,
  ...
  Future<void> startDiagnosis() async {
    emit(state.copyWith(op: Operation.loading, statusMessage: "Starting diagnosis..."));
    ...
    result.fold(
      (error) => emit(state.copyWith(
          op: Operation.failure, statusMessage: error.errorMessage)),
      (response) {
  ...
  Future<void> getSymptomQuestions(String symptomId) async {
    ...
    emit(state.copyWith(
        op: Operation.loading, statusMessage: "Loading questions..."));
    final result =
        await _repo.getSymptomQuestions(state.sessionId!, symptomId);
    result.fold(
      (error) => emit(state.copyWith(
          op: Operation.failure, statusMessage: error.errorMessage)),
      (response) {
  ...
  Future<void> submitSymptomAnswers(
      String symptomId, List<SymptomAnswer> answers) async {
    if (state.sessionId == null) return;
    emit(state.copyWith(
        op: Operation.loading, statusMessage: "Submitting answers..."));
    ...
    result.fold(
      ...
      (response) => emit(state.copyWith(
          op: Operation.success, statusMessage: "Answers submitted")),
    );
  }
  ...
  Future<void> getNextFollowUp() async {
    if (state.sessionId == null) return;
    emit(state.copyWith(
        op: Operation.loading, statusMessage: "Loading next step..."));
  ...
  Future<void> submitFollowUpAnswer(
      String questionId, List<String> selectedIds) async {
    if (state.sessionId == null) return;
    emit(state.copyWith(
        op: Operation.loading, statusMessage: "Submitting..."));
    ...
    result.fold(
      (error) => emit(state.copyWith(
          op: Operation.failure, statusMessage: error.errorMessage)),
      (response) => emit(state.copyWith(
          op: Operation.success, statusMessage: "Answer submitted")),
    );
  }
  ...
  Future<void> getReport() async {
    if (state.sessionId == null) return;
    emit(state.copyWith(
        op: Operation.loading, statusMessage: "Generating report..."));
